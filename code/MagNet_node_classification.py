...
class MagNet_node_classification(nn.Module):
    ...
    def forward(self, real: torch.FloatTensor, imag: torch.FloatTensor, edge_index: torch.LongTensor,
        ...
        # 进行维度扩充，在指定位置加上维数为0的维度
        x = x.unsqueeze(0)
        # 如果三维是permute(0,1,2) 0代表共有几块维度：本例中0对应着3块矩阵 1代表每一块中有多少行：本例中1对应着每块有2行 2代表每一块中有多少列：本例中2对应着每块有5列
        # 这些0,1,2并没有任何实际的意义，也不是数值，只是用来标识区别。有点类似于x，y，z
        # permute(0,2,1) 每一块的行与列进行交换，即每一块做转置行为
        x = x.permute((0, 2, 1))
        x = self.Conv(x)
